...
from instruments import compute_epicenter_weight_matrix, compute_station_travel_time_fields
...
def make_tomography_step(
    ...
    geo_shape = tuple(int(v) for v in geo_grid.shape)
    voxel_size = (float(geo_grid.cell_size),) * 3

    G = []
    r = []

    station_fields = compute_station_travel_time_fields(geo_grid, station_locs, wave_type, solver)
    ...
    for observed in arrivals_table:
        ...
        for weight_idx, weight_val in zip(weights_indices, weights_values):
            ...
            for station_idx, station_loc in enumerate(station_locs):
                station_field = station_fields[station_idx]
                g = _calculate_G(station_field, weight_idx, station_loc, geo_shape, voxel_size)
                G_stations.append(g)

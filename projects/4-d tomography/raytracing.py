import numpy as np
# If your script is in the same folder as velocity_model.py:
from components.utilities import trilinear_interpolation
# velocity_model.py path:
# /Users/podoliako/Documents/New project/GithubEgor_02_2026/projects/4-d tomography/velocity_model.py


def _sample_with_existing_trilinear(values, p_idx):
    """
    values: 3D array
    p_idx: fractional index coords [ix, iy, iz]
    """
...
def trace_ray_from_timefield(
    ...
    for _ in range(max_steps):
        ...
        if ng < 1e-12:
            break
        x = x - step * (g / ng)  # move opposite gradient (toward source)
        path.append(x.copy())
...
def rasterize_path_binary(path_xyz, shape, origin_xyz, spacing_xyz):
    ...
    for a, b in zip(path_xyz[:-1], path_xyz[1:]):
        ...
        n = max(1, int(np.ceil(2.0 * L / spacing.min())))  # oversample
...
def _clip_segment_to_grid(a, b, shape, eps=1e-9):
    """Clip segment [a,b] to box [0,nx)x[0,ny)x[0,nz)."""

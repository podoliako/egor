...
from instruments import prepare_events_x_stations_data, get_travel_times_for_node, get_unique_subset, build_grid_travel_times
from components.utilities import make_grid_3d_time, np
...
def add_grid_ranges(grid, events_df, stations_df):
    events_df['geo_ranges'] = events_df.apply(
    lambda row: grid.find_nodes_in_radius(row['e_lon'], row['e_lat'], 70),
    axis=1
    )

    events_df['time_range'] = events_df.apply(
    lambda row: grid.find_time_range(row['event_dttm'], 15), 
    axis=1
    )

    stations_df['geo_ranges'] = stations_df.apply(
    lambda row: grid.find_nodes_in_radius(row['s_lon'], row['s_lat'], 300),
    axis=1
    )
...
def run_experiment_vp_vs(
    ...
    events_df, stations_df = add_grid_ranges(grid, events_df, stations_df)
    grid_travel_times = build_grid_travel_times(grid, tt_df, events_df, stations_df)


if __name__ == '__main__':
    run_experiment_vp_vs()